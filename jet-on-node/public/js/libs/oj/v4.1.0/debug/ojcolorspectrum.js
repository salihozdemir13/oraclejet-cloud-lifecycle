...
(function ()
{
  ...
  */
  ...
  var  /** @const */  OJES_CONTAINER      = "oj-colorspectrum-container",
       /** @const */  OJES_SPECTRUM       = "oj-colorspectrum-spectrum",
       /** @const */  OJES_THUMB          = "oj-colorspectrum-thumb",
       /** @const */  OJES_HUE_SLIDER     = "oj-colorspectrum-hue",
       /** @const */  OJES_ALPHA_SLIDER   = "oj-colorspectrum-alpha",
       /** @const */  OJES_DISABLED       = "oj-disabled" ;


  // Misc translation keys
  var  /** @const */  TRANSKEY_HUE        = "labelHue",
       /** @const */  TRANSKEY_OPACITY    = "labelOpacity",
       /** @const */  TRANSKEY_SATLUM     = "labelSatLum",
  ...
  oj.__registerWidget("oj.ojColorSpectrum", $['oj']['editableValue'],
  {
    ...
    _AfterCreate: function()
    {
      ...
      if (label)
      {
        ...
        if (!labelId)
        {
          oj.Logger.warn("JET Color Spectrum: The label for this component needs an id in order " +
            "to be accessible");
        }
        else
    ...
    */
    _updateLabelledBy : oj.EditableValueUtils._updateLabelledBy,
     ...
     _onSliderOptionChange: function(e, ui)
     {
       ...
       var  hue, sat, lum, alpha ;
       var  isHueSlider, isAlphaSlider ;
       var  $targ ;
       ...
       if ((! isHueSlider) && (! isAlphaSlider)) { return;}
       ...
       newColor = new oj.Color({"h":hue, "s":sat, "l":lum, "a":alpha}) ;
    ...
    /**
     ...
     _moveThumb : function(xDelta, yDelta)
     {
        ...
        if (x < 0) x = 0 ;
        if (y < 0) y = 0 ;
        if (x >= this._spectrumWidth)  x = this._spectrumWidth -1;
        if (y >= this._spectrumHeight) y = this._spectrumHeight -1;
     ...
     _spectrumClick : function(e)
     {
        ...
        var pos   = this._$spectrum.offset(),    // get click position relative
            xDisp = e.pageX - pos.left,          // relative to spectrum
            yDisp = e.pageY - pos.top,           // top left
            hue, sat, lum,
            o ;
     ...
     {
       var dims = this._$spectrum[0].getBoundingClientRect() ;
     ...
     _handleThumbMoved : function(e, xCenter, yCenter)
     {
        ...
        hue = this._hueVal ;
        o   = this._getSatLumFromPosition(xCenter, yCenter) ;
        sat = o.s;
        lum = o.l;
        ...
        // Fire value optionChange event.
        color = new oj.Color({"h":hue, "s":sat, "l":lum, "a":this._alphaVal});
     ...
     _getSatLumFromPosition : function(xCenter, yCenter)
     {
        ...
        hue = this._hueVal ;
        sat = (xCenter/(this._spectrumWidth-1)) * 100 ;
        lum = 100 - ((yCenter / (this._spectrumHeight-1)) * 100) ;
     ...
     {
       var s         = "hsl",
           haveAlpha = ((typeof alpha === "number") && (alpha < 1));

       s += haveAlpha? "a(" : "(" ;
       hue = Math.round(hue) ;
       sat = Math.round(sat*100)/100 ;
       lum = Math.round(lum*100)/100 ;
       s +=  hue + ", " + sat + "%, " + lum + "%" + (haveAlpha? ("," + alpha) : "") + ")" ;
...
//-------------------------------------------------------------------------------------
        ...
        // Focus ring
        this._focusable({element: this._$spectrumThumb,
                         applyHighlight: true});
    ...
    /**
     ...
     _initData : function()
     {
        ...
        var hueTitle   = this._EscapeXSS(this.getTranslatedString(TRANSKEY_HUE)),
            alphaTitle = this._EscapeXSS(this.getTranslatedString(TRANSKEY_OPACITY)),
            thumbDesc  = this._EscapeXSS(this.getTranslatedString(TRANSKEY_THUMB_DESC));
    ...
    /**
     ...
     _clear : function()
     {
        ...
        this._$alphaSlider      =
        this._$spectrum         =
        this._$spectrumThumb    =
        this._spectrumThumbSize =
        this._strings           =
